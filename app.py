...
================================================
Upload: Amazon Inventory Ledger (required) + MTR Sales (optional)

Sales  : abs(Customer Shipments) − Customer Returns  [from Inventory Ledger]
Stock  : Ending Warehouse Balance — latest row per SKU / FC / Disposition
FC-wise: every plan table has one stock column per active FC
75     : verified Amazon India FC codes built-in
MTR    : optional — supplements only SKUs absent from ledger
"""
...
[data-testid="stMetricLabel"]   { font-size:.8rem; color:#555 }
.ab { padding:.45rem 1rem; border-radius:6px; margin:.2rem 0;
...
.ab-b { background:#ebf8ff; border-left:4px solid #3182ce; color:#1a365d }
.stTabs [data-baseweb="tab"] { font-size:13px; font-weight:600 }
thead tr th { background:#1a365d !important; color:white !important;
...
st.caption(
    "Inventory Ledger → correct sales (Customer Shipments) · "
    "FC-wise stock · dispatch plan · risk alerts · Amazon flat file"
)
...
#   DEL8_DED5  = single physical FC in Sohna, Haryana
#   HYD8_HYD3  = single physical FC in Shamshabad, Telangana
# ══════════════════════════════════════════════════════════════════════════════
FC_DATA = {
    # Assam
...
def alert(msg, style="g"):
    st.markdown(f'<div class="ab ab-{style}">{msg}</div>', unsafe_allow_html=True)
...
def excel_sheet(writer, df, sheet_name):
    ...
    for ci, cn in enumerate(d.columns):
        ...
        mx = d[cn].astype(str).str.len().max() if not d.empty else 8
        sh.set_column(ci, ci, min(max(len(str(cn)), mx) + 2, 42))
    sh.freeze_panes(2, 0)
...
with st.sidebar:
    ...
    sales_basis   = st.selectbox(
        "Sales Basis for Avg Daily Sale",
    ...
    foc_cluster  = st.multiselect("Focus Clusters (blank = all)", all_clusters)
...
with cu1:
    inv_file = st.file_uploader(
        "🏭 Amazon Inventory Ledger — Required  (CSV / ZIP / XLSX)",
        type=["csv", "zip", "xlsx"],
    )
with cu2:
    mtr_files = st.file_uploader(
        "📊 MTR / Sales Report — Optional  (CSV / ZIP / XLSX, multiple OK)",
        type=["csv", "zip", "xlsx"],
        accept_multiple_files=True,
    )

with st.expander("📋 File Formats & How Sales / Stock Are Calculated"):
    st.markdown("""
**Inventory Ledger** — Seller Central → Reports → Fulfillment → Inventory Ledger

| Column | Example | Purpose |
|---|---|---|
| `Date` | 02/12/2026 | Daily dedup — keep latest row per SKU/FC |
| `FNSKU` | X00275KJZP | Amazon flat file |
| `MSKU` | BR-899 | SKU key |
| `Title` | GLOOYA Bracelet | Display name |
| `Disposition` | SELLABLE / CUSTOMER_DAMAGED | Sellable vs damaged |
| `Ending Warehouse Balance` | 18 | **✅ Correct Stock** (latest row per SKU/FC) |
| `Customer Shipments` | -3 | **✅ Correct Sales** (negative = shipped out) |
| `Customer Returns` | 1 | Subtracted from shipments |
| `Location` | LKO1 | FC code |

**Sales formula:** `Net Units Sold per day = abs(Customer Shipments) − Customer Returns`

**Stock formula:** `Sellable Stock = Ending Warehouse Balance` from the **latest date only** per SKU/FC

> MTR file is optional — only fills SKUs not present in the Inventory Ledger.
""")

if not inv_file:
    st.info("👆 Upload the **Inventory Ledger** above to start. MTR is optional.")
    with st.expander("🗺️ All 75 Verified Amazon India FC Codes — reference"):
        st.dataframe(
            pd.DataFrame([{
...
C_SHIP  = find_col(inv_raw, ["Customer Shipments"],                        ["customer shipment"])
C_RET   = find_col(inv_raw, ["Customer Returns"],                          ["customer return"])
...
for lbl, col in [("SKU/MSKU", C_SKU), ("Ending Balance", C_QTY), ("Location/FC", C_LOC)]:
    if not col:
        ...
        st.write("Columns found:", list(inv_raw.columns))
        st.stop()
...
inv["Disposition"] = (inv[C_DISP].astype(str).str.upper().str.strip()
                      if C_DISP else "SELLABLE")

# ── DAILY DEDUP — keep LATEST row per SKU/FC/Disposition ─────────────────────
# Ledger = 1 row per day → summing = Stock × days (wrong). Fix: .last() per group.
n_raw = len(inv)
if C_DATE:
...
# ── FC-wise stock pivot — one column per active FC ───────────────────────────
...
fc_pivot      = (_piv
    .pivot_table(index="MSKU", columns="col", values="Stock",
...
#            Net Daily Sales = abs(Customer Shipments) − Customer Returns
# SECONDARY: MTR file (optional) fills SKUs absent from ledger
# ══════════════════════════════════════════════════════════════════════════════
ledger_sales = pd.DataFrame()
if C_SHIP and C_DATE:
    ls            = inv_raw.copy()
    ls["MSKU"]    = ls[C_SKU].astype(str).str.strip()
    ls["Date"]    = pd.to_datetime(ls[C_DATE], dayfirst=True, errors="coerce")
    ls["Shipped"] = pd.to_numeric(ls[C_SHIP], errors="coerce").fillna(0).abs()
    ls["Returns"] = (pd.to_numeric(ls[C_RET], errors="coerce").fillna(0).clip(lower=0)
                     if C_RET else 0)
...
if mtr_files:
    _raw = [read_file(f) for f in mtr_files]
    _raw = [d for d in _raw if not d.empty]
    if _raw:
        sr      = pd.concat(_raw, ignore_index=True)
        MS_SKU  = find_col(sr, ["Sku", "SKU", "MSKU", "Item SKU"],         ["sku", "msku"])
        MS_QTY  = find_col(sr, ["Quantity", "Qty", "Units"],               ["qty", "unit", "quant"])
        MS_DATE = find_col(sr, ["Shipment Date", "Purchase Date",
                                 "Order Date", "Date"],                    ["date"])
        MS_FT   = find_col(sr, ["Fulfilment", "Fulfillment",
                                 "Fulfilment Channel",
                                 "Fulfillment Channel"],                   ["fulfil", "channel"])
        if MS_SKU and MS_QTY and MS_DATE:
            FT_MAP      = {"AFN": "FBA", "FBA": "FBA", "AMAZON_FULFILLED": "FBA",
                           "MFN": "FBM", "FBM": "FBM", "MERCHANT_FULFILLED": "FBM"}
            ms            = sr.copy()
            ms["MSKU"]    = ms[MS_SKU].astype(str).str.strip()
            ms["Qty"]     = pd.to_numeric(ms[MS_QTY], errors="coerce").fillna(0)
            ms["Date"]    = pd.to_datetime(ms[MS_DATE], dayfirst=True, errors="coerce")
            ms["Channel"] = (ms[MS_FT].astype(str).str.upper().str.strip()
                             .map(FT_MAP).fillna("FBA") if MS_FT else "FBA")
            mtr_sales     = (ms[(ms["Qty"] > 0) & ms["Date"].notna()]
                             [["MSKU", "Date", "Qty", "Channel"]].copy())

if not ledger_sales.empty and not mtr_sales.empty:
    mtr_extra    = mtr_sales[~mtr_sales["MSKU"].isin(set(ledger_sales["MSKU"]))]
    sales        = pd.concat([ledger_sales, mtr_extra], ignore_index=True)
    sales_source = "Inventory Ledger (Customer Shipments) + MTR supplement"
elif not ledger_sales.empty:
    sales        = ledger_sales.copy()
    sales_source = "Inventory Ledger — Customer Shipments column"
elif not mtr_sales.empty:
    sales        = mtr_sales.copy()
    sales_source = "MTR Sales Report only"
else:
    st.error(
        "No sales data found. "
        "Ensure Inventory Ledger has a 'Customer Shipments' column, or upload an MTR file."
    )
    st.stop()
...
for c in ["Hist Sales", "All-Time Sales", "Total Sellable",
          "Total Damaged", "Demand StdDev"]:
...
fcp = fc_stock_long.merge(
    plan[["MSKU", "Channel", "Avg Daily Sale", "Dispatch Needed",
          "Total Sellable", "Required Stock", "Title", "FNSKU",
          "Priority", "Days of Cover"]],
    on="MSKU", how="left",
)
fcp["FC DOC"]     = np.where(
...
fcv = fcp.copy()
if foc_cluster:
    fcv = fcv[fcv["FC Cluster"].isin(foc_cluster)]
...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 7 — FC VELOCITY
# ══════════════════════════════════════════════════════════════════════════════
fc_velocity = (plan[["MSKU", "Title", "Avg Daily Sale", "Velocity"]]
    .drop_duplicates("MSKU")
    .merge(
        fc_stock_long[["MSKU", "FC Code", "FC Name", "FC City", "FC Cluster", "FC Stock"]],
        on="MSKU", how="left",
    )
...
m4.metric("Total Sold",        fmt(tot_sold))
m5.metric("Units to Dispatch", fmt(tot_disp))
m6.metric("🔴 Critical SKUs",  fmt(len(r_crit)))
m7.metric("Avg Days of Cover", f"{avg_doc:.0f}d" if pd.notna(avg_doc) else "N/A")

alert(
    f"✅  Stock as of <b>{inv_date_lbl}</b>  |  "
    f"Sales: <b>{sales_source}</b>  |  "
    f"{n_dedup} rows after daily dedup (raw: {n_raw})  |  "
    f"FCs: {', '.join(_active_fcs)}", "g")

if r_crit.shape[0]:
    alert(f"🚨  {len(r_crit)} SKU(s) below 14 days of stock — replenish immediately!", "r")
if r_dead.shape[0]:
    alert(f"⚠️  {len(r_dead)} SKU(s) have stock but ZERO sales — fix listing or liquidate.", "y")
if r_excess.shape[0]:
    alert(f"📦  {len(r_excess)} SKU(s) overstocked (>{planning_days*2}d) — pause replenishment.", "y")
if tot_dmg:
    alert(f"🔧  {fmt(tot_dmg)} damaged units — raise Removal/Reimbursement in Seller Central.", "y")
...
with tabs[0]:
    ...
    st.info(
        f"**Sales source:** {sales_source}  \n"
        f"**Sales range:** {s_min.strftime('%d %b %Y')} → {s_max.strftime('%d %b %Y')} "
        f"({up_days} days)  |  **Basis used:** {sales_basis} ({h_days} days)  \n"
    ...
    with oa:
        ...
        top5 = (plan[plan["Dispatch Needed"] > 0].nlargest(5, "Priority")
                [["MSKU", "Title", "Avg Daily Sale",
                  "Days of Cover", "Dispatch Needed", "Priority"]].copy())
        top5["Title"] = top5["Title"].apply(lambda x: trunc(x, 38))
    ...
    with ob:
        ...
        if not mo_tr.empty:
            ...
            st.bar_chart(pm)
    st.markdown("**🏭 FC-Wise Inventory Summary**")
...
with tabs[1]:
    ...
    st.markdown("**📊 Stock & Dispatch Summary by FC**")
    fc_rows = []
    ...
    if fc_rows:
        st.dataframe(
            sno(pd.DataFrame(fc_rows).sort_values("Total Stock in FC", ascending=False)),
            use_container_width=True, hide_index=True)
...
with tabs[5]:
    ...
    rt1, rt2, rt3, rt4 = st.tabs([
        "🔴 Critical (<14d)", "⚫ Dead Stock",
        "🟡 Slow Moving (>90d)", "🔵 Excess"])

    with rt1:
        st.markdown(f"**{len(r_crit)} SKU(s) — replenish immediately**")
        if not r_crit.empty:
    ...
    with rt2:
        ...
        if not r_dead.empty:
            ...
            c2 = (["MSKU", "Title", "Channel", "Total Sellable"]
                  + FC_STOCK_COLS + ["All-Time Sales"])
            st.dataframe(sno(d2[[c for c in c2 if c in d2.columns]]),
                         use_container_width=True)
        else:
    ...
    with rt3:
        ...
        if not r_slow.empty:
            st.dataframe(
                risk_view(r_slow.sort_values("Days of Cover", ascending=False)),
                use_container_width=True)
        else:
    ...
    with rt4:
        ...
        if not r_excess.empty:
            st.dataframe(
                risk_view(r_excess.sort_values("Days of Cover", ascending=False)),
                use_container_width=True)
        else:
...
with tabs[6]:
    ...
    with tc1:
        ...
        if not wk_tr.empty:
            st.line_chart(wk_tr.set_index("Week")["Units Sold"])
    with tc2:
        st.markdown("**📅 Monthly Sales by Channel**")
        if not mo_tr.empty:
            pm = mo_tr.pivot_table(index="Month", columns="Channel",
    ...
    st.divider()
    st.markdown("**📊 FC-Wise Stock Velocity**")
    ...
    sel_sku = st.selectbox("Select MSKU", sorted(plan["MSKU"].unique()), key="drill")
    sku_d   = (sales[sales["MSKU"] == sel_sku]
               .groupby("Date")["Qty"].sum().reset_index().set_index("Date"))
    if not sku_d.empty:
        st.caption(f"**{trunc(title_map.get(sel_sku, sel_sku), 80)}**")
        st.line_chart(sku_d["Qty"])
    sku_fc = fc_stock_long[fc_stock_long["MSKU"] == sel_sku]
    if not sku_fc.empty:
        st.markdown(f"**FC Stock Breakdown for {sel_sku}**")
        st.dataframe(
            sku_fc[["FC Code", "FC Name", "FC City", "FC State", "FC Cluster", "FC Stock"]],
            use_container_width=True, hide_index=True)
    else:
        st.info("No FC stock data for this SKU.")
...
with tabs[7]:
    ...
    if dmg_detail.empty:
        ...
        st.dataframe(sno(dr.sort_values("Qty", ascending=False)),
                     use_container_width=True)
        st.divider()
        st.markdown("**📋 Disposition Breakdown per MSKU**")
        db = disp_bkdn.copy()
        db.insert(1, "Title",
...
with tabs[8]:
    ...
    st.info(
        ...
        "_Each FC needs its own separate flat file._")
    ...
    st.markdown(
        f"**{len(flat_base)} SKUs  |  "
        f"{fmt(flat_base['Units to Ship'].sum())} total units to dispatch**")
    ...
    def make_flat_file(df, fc_code):
        ...
        mh    = ["ShipmentName", "ShipFromName", "ShipFromAddressLine1",
        ...
        for _, r in df.iterrows():
            ...
            lines.append("\t".join([
                ...
                str(int(r["Units to Ship"])), qic,
                prep_own, lbl_own,
                str(r.get("Title Short", ""))[:200],
                str(shp_date),
            ]))
        return "\n".join(lines)
    ...
    if len(_active_fcs) > 1:
        st.divider()
        st.markdown("**📦 Download one flat file per FC**")
...
def build_excel():
    ...
    with pd.ExcelWriter(buf, engine="xlsxwriter") as w:
        ...
        excel_sheet(w, risk_view(r_excess.sort_values("Days of Cover", ascending=False)),
                    "Excess — FC Stock")
        _t20c = (["MSKU", "Title", "Avg Daily Sale", "Total Sellable"]
                 + FC_STOCK_COLS + ["Days of Cover", "Velocity", "Dispatch Needed"])
        excel_sheet(w, sno(r_top20[[c for c in _t20c if c in r_top20.columns]]),
        ...
        _vc = ["FC Code", "FC Name", "FC City", "FC Cluster",
               "MSKU", "Title", "FC Stock", "Avg Daily Sale", "Velocity"]
        excel_sheet(w, sno(fc_velocity[[c for c in _vc if c in fc_velocity.columns]]),
                    "FC Velocity")
        # Damaged & trends
...
with dl2:
    _csv = fba_plan[fba_plan["Dispatch Needed"] > 0].copy()
    _csv["Title"] = _csv["Title"].apply(lambda x: trunc(x, 80))
    _csv_cols = [c for c in PLAN_COLS if c in _csv.columns]
    st.download_button(
        "📋 Download Dispatch Plan — CSV (FC-wise)",
        data=_csv[_csv_cols].to_csv(index=False),
